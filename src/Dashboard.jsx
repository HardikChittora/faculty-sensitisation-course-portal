...
import { BookOpen, CheckCircle, GraduationCap, LogOut, ArrowLeft, ChevronDown, ChevronRight, PlayCircle, FileText, Award } from 'lucide-react';
import CoursePlayer from './CoursePlayer';

const COURSES = [
  { id: 'c1', title: 'Faculty Sensitization', description: 'Core principles of modern teaching, diversity, and academic integrity for higher education faculty.', totalModules: 3, image: 'https://images.unsplash.com/photo-1524178232363-1fb2b075b655?q=80&w=2070&auto=format&fit=crop' }
];
...
export default function Dashboard({ user, onLogout }) {
  ...
  const updateModuleProgress = (modKey, updates) => {
    setProgress(prev => {
      const updated = { ...prev };
                ...
                <div key={modKey} style={{ marginBottom: '12px' }}>
                  ...
                  >
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                      {modData.passed ? <CheckCircle size={18} color="var(--success)" /> : (isLocked ? <div style={{width:'18px',height:'18px',borderRadius:'50%',border:'2px solid #cbd5e1'}}/> : <div style={{width:'18px',height:'18px',borderRadius:'50%',border:'2px solid var(--primary)'}}/>)}
                      Module {modNum}
                    </div>
  ...
  const renderMainArea = () => {
    if (currentView === 'course' && activeModule) {
      return (
        <CoursePlayer 
        ...
        <div className="course-catalog">
          ...
          {completedCourses.length > 0 ? (
                ...
                <div key={course.id} className="course-card big-card" style={{ borderLeft: '4px solid var(--success)' }}>
                  ...
                  <div className="card-content">
                    ...
                    <p style={{ color: 'var(--success)', fontWeight: '500', display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '16px' }}>
                      <CheckCircle size={16} /> Fully Completed
                    </p>
                    <button 
                      style={{ 
            ...
            <div className="course-empty-state" style={{ marginTop: '60px' }}>
              ...
              <p>Finish all modules in a course to earn your completion status.</p>
            </div>
          )}
        </div>
      );
